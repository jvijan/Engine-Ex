...
public:
	...
	{
		/* CHANGED: Don't need autograder mode for this now
		if (IsAutograderMode())
		{
			x = 0;
			y = 0;
	...
	{
		// CHANGED: Disable vsync for full speed
		if (maxSpeed) //IsAutograderMode())
	...
	static void SDL_RenderPresent(SDL_Renderer* renderer)
	{
		...
		// CHANGED: Disable frame recording for now, don't care about data
#if 0
		...
		// CHANGED: Always present, autograder not here anymore
		//if (!_autograder_mode)
	...
	static void SDL_RenderCopyEx(int actor_id, const std::string& actor_name, SDL_Renderer* renderer, SDL_Texture* texture, const SDL_FRect* srcrect, const SDL_FRect* dstrect, const float angle, const SDL_FPoint* center, const SDL_RendererFlip flip)
	{
		...
		// CHANGED: Stopped render logs for now for better performance
#if 0
...
private:
	...
	static void SDL_ConsiderInputFile()
	{
		...
		// CHANGED:: Removed recording mode for now
#if 0
	...
	static void SDL_Delay() {
		...
		// CHANGED: Don't delay if we want to text max speed for benchmarking
		if (maxSpeed)//_autograder_mode)
		{
